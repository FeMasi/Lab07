...
class Model:
    ...
    def _ricorsione(self, parziale, situazioni):
        ...
        if len(parziale) ==15:
            ...
            for situazione in situazioni[(day-1)*3: day*3]:
                #if situazione.data.day == day sarebbe la stssa cosa
    ...
    def _vincoli_soddisfatti(self, parziale, situazione) -> bool:
        ...
        #se la sequenza ha uno o due elementi posso solo rimettere il primo
        ...
        #e vedere se il tecnico si è fermato almeno 3 giorni di fila nello stesso posto
        elif len(parziale)>2:
            sequenza_finale = parziale[-3:] # <- ultimi 3 giorni in parziale
            prima_fermata = sequenza_finale[0].localita # <- prmo di questi ultimi 3 giorni
            counter = 0
